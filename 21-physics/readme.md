...
const playHitSound = (collision) => {
  // check impact velocity. without it, if there is a small collision, it keeps making sound.
  const impactStrength = collision.contact.getImpactVelocityAlongNormal();

  if (impactStrength > 1.5) {
    // make the sound natural
    hitSound.volume = Math.random();
    // prevent sound  from playing before collision
...
// The broadphase is doing a rough sorting of the Bodies before testing them.
...
// Materials - set frictiion and bouncing
...
const defaultContactMaterial = new CANNON.ContactMaterial(
  ...
  {
    friction: 0.1,
    restitution: 0.7, // return, restoration
  }
);
world.addContactMaterial(defaultContactMaterial); // remind!
world.defaultContactMaterial = defaultContactMaterial;

// Floor - make static floor!
...
// floor.rotation.x = -Math.PI * 0.5; -> floor is rotated.
...
const createBox = (width, height, depth, position) => {
  ...
  // Cannon.js body - the length should be half because it starts from center
...
const tick = () => {
  ...
  for (const object of objectsToUpdate) {
    ...
    // quaternion makes the movement of object natural in case of collisions and frictions. changes of angles.
...
workers: https://schteppe.github.io/cannon.js/examples/worker.html
